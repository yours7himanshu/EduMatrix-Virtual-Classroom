...
const AdminLive = () => {
  ...
  const handleNegotiationNeeded = useCallback(async () => {
    ...
    if (!remoteEmailId || !peer || !socket?.connected) {
        console.warn("Skipping renegotiation: missing remoteEmailId, peer, or socket connection.");
        return;
    }
    try {
        ...
        console.error("Error during renegotiation offer creation/sending:", error);
    }
  ...
  const handleUserLeft = useCallback(({ emailId }) => {
    ...
    if (emailId === remoteEmailId) {
       console.log("Matching user left. Clearing remote participant information.");
  ...
  useEffect(() => {
    ...
    const handleIceCandidateEvent = ({ candidate }) => {
      if (candidate && peer.signalingState !== 'closed') {
          ...
          peer.addIceCandidate(new RTCIceCandidate(candidate)).catch(e => console.error("Error adding received ICE candidate:", e));
      }
    };
  ...
  }, [socket, peer, handleNewUserJoined, handleIncommingCall, handleCallAccepted, handleUserLeft, createAnswer, remoteEmailId]);
    ...
    const loadModel = async () => {
      try {
        ...
        if (isMounted) {
          ...
          if (localVideoRef.current?.readyState >= localVideoRef.current?.HAVE_METADATA && !isDetectionRunningRef.current) {
             console.log("Model loaded after metadata, attempting to start detection.");
  ...
  const detectFaceLoop = useCallback(async () => {
    ...
    if (!isDetectionRunningRef.current || !localVideoRef.current || !modelRef.current || isDetectingRef.current) {
    ...
    if (videoElement.readyState < videoElement.HAVE_METADATA) {
        console.warn("detectFaceLoop called but metadata not ready, rescheduling.");
    ...
    try {
      ...
      if (predictions.length === 0) {
         ...
         currentAlert = "More than one face detected by our system! You are cheating";
      } else {
         currentAlert = ""; 
      }
      ...
      setAlertMsg(prev => prev !== "Error: Face detection failed." ? "Error: Face detection failed." : prev);
  ...
  const handleLocalVideoMetadataLoaded = useCallback(() => {
    ...
    if (modelRef.current && !isDetectionRunningRef.current && localVideoRef.current) {
        console.log("Model ready and metadata loaded. Starting face detection loop.");
        ...
        if (localVideoRef.current.videoWidth > 0 && localVideoRef.current.videoHeight > 0) {
            ...
            detectFaceLoop(); // Start the loop
             ...
             console.warn("Metadata loaded, but video dimensions not available yet. Detection not started.");
             
        }
    } else if (!modelRef.current) {
        console.warn("Metadata loaded, but face detection model not ready yet.");
    } else if (isDetectionRunningRef.current) {
        console.log("Metadata loaded, but detection loop appears to be already running.");
  ...
  useEffect(() => {
      ...
      if (peer && myStream && sendStream && (peer.connectionState === 'new' || peer.connectionState === 'connecting' || peer.connectionState === 'connected') ) {
  ...
  return (
    <div className="video-call flex h-screen w-screen bg-gray-900 text-white"> {/* Added text-white for base color */}
      
      <div className="flex w-[70%] flex-col items-center bg-gray-800 p-4 md:p-6 rounded-lg shadow-lg overflow-hidden m-2"> 
        ...
        <div className="h-10 mb-2 flex items-center justify-center w-full"> 
             {alertMsg && (
                <p className={`p-2 rounded text-sm md:text-base transition-opacity duration-300 ${alertMsg.startsWith('Error:') ? 'bg-red-500 text-white' : 'bg-yellow-500 text-black'}`}> 
                    {alertMsg}
                </p>
             )}
        </div>
        ...
        <div className={`relative w-full flex-grow grid gap-4 ${remoteStream ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'}`}>
          ...
          <div className={`relative bg-black rounded-lg overflow-hidden shadow-md border-2 ${alertMsg.includes("detected") || alertMsg.includes("watching") ? 'border-yellow-500' : 'border-green-500' }`}> {/* Dynamic border */}
            <video
              ref={localVideoRef}
            ...
            <span className="absolute bottom-1 left-1 md:bottom-2 md:left-2 bg-black bg-opacity-60 text-white text-xs px-1 rounded">You</span>
          </div>
          ...
          {remoteStream && (
            <div className="relative bg-black rounded-lg overflow-hidden shadow-md border-2 border-blue-500">
              ...
              />
              {remoteEmailId && <span className="absolute bottom-1 left-1 md:bottom-2 md:left-2 bg-black bg-opacity-60 text-white text-xs px-1 rounded">{remoteEmailId}</span>}
            </div>
          )}
